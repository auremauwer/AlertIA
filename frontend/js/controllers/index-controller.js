...
class IndexController {
    ...
    async handleFileSelect(file) {
        ...
        try {
            ...
            // Obtener todas las obligaciones existentes para verificar duplicados por ID
            const todasObligaciones = await this.obligacionesService.getAll();
            const obligacionesExistentes = new Set(
                todasObligaciones.map(obl => obl.id ? String(obl.id).trim().toLowerCase() : '')
            );
            ...
            const duplicadosOmitidos = []; // Array para guardar IDs duplicados

            for (const obligacion of obligaciones) {
                try {
                    // Verificar si ya existe una obligación con el mismo ID
                    // Usamos ID como criterio único según solicitud del usuario
                    if (!obligacion.id) {
                        // Si por alguna razón pasó sin ID, generamos uno temporal para procesarla
                        // pero idealmente ya deberían venir filtradas
                        console.warn('Obligación sin ID detectada en el bucle de guardado');
                    }

                    const clave = obligacion.id ? String(obligacion.id).trim().toLowerCase() : null;

                    if (clave) {
                        // Verificar duplicados en la base de datos existente o en la carga actual
                        if (obligacionesExistentes.has(clave) || obligacionesEnCarga.has(clave)) {
                            console.log(`[Duplicado omitido] ID repetido: ${clave}`);
                            duplicadosOmitidos.push({
                                id: obligacion.id,
                                razon: 'ID duplicado en base de datos o archivo'
                            });
                            skipped++;
                            continue;
                        }
            ...
            try {
                ...
                const configToSave = {
                    ...
                    nombre_archivo_excel: file.name, // Guardar nombre del archivo
                    ultima_carga_excel: new Date().toISOString(),
                    filas_excluidas: filasExcluidas, // Guardar filas excluidas con sus razones
                    duplicados_omitidos: duplicadosOmitidos // Guardar duplicados omitidos
                };
